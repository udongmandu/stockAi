...
if st.session_state.chat_open:
    ...
    if question:
        ...
        아래는 최근 {days}일 동안 {stock} 관련 뉴스 데이터야.
        반드시 이 데이터를 근거로 종합적으로 분석해서 답변해.
        하지만 주식관련 질문이 아니라면 데이터를 통한 대답 보다 일반적으로 대답해도 돼.
        데이터에 없는 내용은 절대 절대 추측하지 말고 '관련 뉴스 없음'이라고 말하세요.
        power 값은 해당 뉴스의 영향력을 예측한 값이니까 만약 질문이 예측해줘 혹은 추천해줘 이런내용이면 이 power 값을 참조하고
        가장 영향을 준 기사를 보여주는 것도 좋아. 하지만 제목만 알려주고 URL은 DB에 적재하지 않으니까 URL은 주지마
        데이터:
        {context}
        """
        payload = {
