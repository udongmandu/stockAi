...
if st.session_state.chat_open:
    ...
    if question:
        ...
        answer_prompt = f"""
        당신은 금융 애널리스트입니다. 사용자가 "{question}" 라고 물었습니다.

        아래는 최근 {days}일 동안 {stock} 관련 뉴스 데이터입니다.
        이 데이터를 반드시 근거로 분석하여 답변하세요.

        규칙:
        1. 데이터에 없는 내용은 절대 추측하지 말고 "관련 뉴스 없음"이라고만 답하세요.
        2. 질문이 주식과 관련 없다면 일반 상식 수준으로 답하세요.
        3. "추천" 또는 "예측" 관련 질문일 경우:
        - power 값이 높은 뉴스일수록 더 큰 영향력을 준다고 판단하세요.
        - 가장 power가 큰 기사 1~2개를 근거로 제시하세요.
        - 기사 제목만 알려주고, URL은 주지 마세요. (DB에 없으므로)
        4. 답변은 아래 형식을 따라라:
        - 요약 분석 (2~3문장)
        - 주요 기사 제목 (최대 2개)
        - 결론/추천 (1문장)
